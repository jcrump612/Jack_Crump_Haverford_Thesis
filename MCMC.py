...
def SN_Chisq_Proper(params):
    ...
    #Set the priors
    if H0<40. or H0>90. or Omega_m<0.001 or Omega_m>0.5 or omega<=0 or omega>10000 or zeta<-1 or zeta>1 or np.isnan(H0)==True or np.isnan(Omega_m)==True or np.isnan(omega)==True or np.isnan(zeta)==True: 
        return np.inf
...
p0 = np.random.normal([67.4,0.315,5000,1e-100], [5,0.1,1000,0.5],size=(nwalkers,ndim))
